<div align="center">

# ARINYA MAURYA

**Software Developer | Crafting Scalable Digital Experiences**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-9B59B6?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arinya-maurya/)
[![Email](https://img.shields.io/badge/Email-E0938E?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mauryaarinya57@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=vercel&logoColor=white)](https://portfolio-five-theta-zn0eqpf8sl.vercel.app/)

> 🚀 **Actively seeking Full Stack / Frontend roles** — available to join immediately!

</div>

---

### 👩‍💻 About Me

- ⚡ **Full Stack Developer** with 1+ year of experience building production-ready apps.
- 🌱 **Current Focus:** Shipping reusable UI libraries and optimizing database performance.
- 🎯 **Learning:** Leveling up in System Design to build more resilient, scalable architectures.
- 💼 **Open to:** Full-time roles, contract work, and freelance projects — **immediate availability**
- 📬 **Reach out:** [mauryaarinya57@gmail.com](mailto:mauryaarinya57@gmail.com)

---

### 💎 What I Bring to Your Team

| | Skill | Impact |
|:---:|:---|:---|
| 🎨 | Frontend (React, Next.js, Tailwind) | Pixel-perfect, responsive UIs that users love |
| ⚙️ | Backend (Node.js, Express, REST APIs) | Scalable APIs with clean architecture |
| 🗄️ | Databases (MongoDB, MySQL) | Efficient schema design & query optimization |
| 🚀 | Deployment (Vercel, Git) | CI/CD-ready, production-grade shipping |

---

### ⚙️ Tech Stack

#### **Core & Frontend**
<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,tailwind,bootstrap,js,ts,react,nextjs,angular&perline=10" />
</p>

#### **Backend & Databases**
<p align="left">
  <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,mysql&perline=10" />
</p>

#### **Tools & Concepts**
<p align="left">
  <img src="https://skillicons.dev/icons?i=git,github,vscode,vercel&perline=10" />
  <br>
  <code>OOPs</code> • <code>SQL</code> • <code>REST APIs</code> • <code>System Design</code>
</p>

---

### 🚀 Featured Projects

| Project | Description | Stack | Live |
|:---|:---|:---|:---:|
| 🌐 [Personal Portfolio](https://github.com/Arinyamaurya/Portfolio) | Modern portfolio showcasing technical skills and creative work. | Next.js, Tailwind CSS | [Demo ↗](https://portfolio-five-theta-zn0eqpf8sl.vercel.app/) |
| 🎥 [Zoomy Video Call](https://github.com/Arinyamaurya/Zoomy_video_confrencing) | Real-time video conferencing app with secure room management. | React, WebRTC, Node.js | [Demo ↗](https://zoomy-video-confrencing.vercel.app/) |
| 🎲 [Number Guessing](https://github.com/Arinyamaurya/number_guessing_game) | Logic-based game focused on algorithm efficiency and state handling. | JavaScript, CSS | [Demo ↗](https://number-guessing-game-rho-blue.vercel.app/) |

--- 

### 💡 *Code. Build. Improve. Repeat.*

</div>
